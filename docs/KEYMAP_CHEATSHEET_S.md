...
- `;/Scrl` = `&scroll_hold 4 SEMI`（タップ=`;`、140msホールド中=スクロール）
...
- スクロールは `;` を140ms以上ホールドしながらボールを動かします。放すと即座に通常カーソルへ戻ります。
...
- 全キー透過。**役割はキー入力ではなく「`;` のホールド中だけトラックボール移動をスクロールに変換」**（`zip_xy_scaler 1 6` で1/6減速 → `zip_xy_to_scroll_mapper`）。
...
| **スクロール** | **`;` を140msホールドしながらボール移動**（放すと即解除） |
