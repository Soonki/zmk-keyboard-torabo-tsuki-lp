...
| 左クリック | `J` + `K` |
| 右クリック | `K` + `L` |
| スクロール | `H` + `L` を一度タップしてからトラックボールを動かす |
| ブラウザで戻る（MB4） | `N` + `M` |
| ブラウザで進む（MB5） | `,` + `.` |
...
- スクロールは次の通常キー入力、または5秒経過で自動解除されます。解除時に押した通常キーもそのまま入力されます。
...
## Layer 4 — ワンショットスクロール（`H` + `L` コンボで起動）
...
| **スクロール** | **`H` + `L` を一度タップしてからボール移動**（次のキーまたは5秒で解除） |
