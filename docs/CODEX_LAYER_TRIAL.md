...
そのため、現在書き込まれているファームウェアのマウスコンボはLayer 1でも有効なままです。試用中は `H+J`、`J+K`、`K+L`、`L+;`、`,+.` を同時押ししないでください。

リポジトリ側の `config/keymap.keymap` では、恒久版としてマウスコンボからLayer 1を除外済みです。この分離を有効にするには、次回生成したファームウェアの書き込みが必要です。
